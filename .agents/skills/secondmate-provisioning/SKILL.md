...
TraeX is verified only for a local tmux secondmate agent. Its primary home's `config/traex-adapter` must explicitly open both `secondmate=on` and `primary=on`, and its native-trust receipt must pass before the local endpoint is created. A TraeX remote route is an explicit refusal before SSH, remote readiness, or any Herdr lifecycle action; do not fall back to another placement. `config/traex-adapter` is intentionally not inherited because propagating a local role gate could falsely authorize that excluded remote axis.
