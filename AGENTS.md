...
config/traex-adapter  explicit role-separated TraeX feature gate (`worker=`, `primary=`, `secondmate=`); LOCAL, gitignored, not inherited, and valid only after native hook trust plus the binary-bound receipt in the TraeX CLI home; see docs/configuration.md "TraeX adapter"
...
state/               volatile runtime signals; gitignored
  ...
  .traex-primary-hook-token .traex-primary-session  TraeX primary binding and native session identity; owned by the trusted global dispatcher
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, and `kimi`, plus feature-gated `traex` for ordinary workers, scouts, the primary, and local tmux secondmates, and `muse` for crewmates and scouts only; never dispatch on an unverified adapter or on a TraeX backend/role outside that exact set.
