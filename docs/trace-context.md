...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `muse`, plus Secondmate spawns across that same set except the deliberately crewmate-only `muse` adapter. It also covers feature-gated TraeX workers, scouts, and local tmux Secondmates; that adapter refuses every non-tmux or remote placement before reaching this site.
This is the same coverage `GOTMPDIR` already has and requires no trace-specific `launch_template()` behavior.
Apart from TraeX's narrower boundary, ship and scout spawns reach that site on every spawn backend (`tmux`, `herdr`, `zellij`, `orca`, `cmux`); a Secondmate reaches it on every backend that accepts a Secondmate spawn (`tmux`, `herdr`, `zellij`), because `bin/fm-spawn.sh` rejects a Secondmate on `orca` and `cmux`.
