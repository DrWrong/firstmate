...
Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Muse, and feature-gated TraeX share that backend cleanup boundary; their harness-specific hook files, tokens, and session-log sidecars are cleaned only after it, so no harness needs a separate endpoint parser.
[`traex.md`](traex.md) owns TraeX's separate local-tmux binding and exact-target cleanup evidence.
